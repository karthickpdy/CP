...
# # 	sorted = Hash[*arr.sort.reverse.each_cons(2).to_a.flatten]

# # 	# puts "#{sorted}"
# # 	(n-1).times do 
# # 		a,b = gets.split(" ").map(&:to_i)
# # 		x = -1
# # 		x = sorted[planets[b-1]] if planets[b-1] == arr[b-1]
# # 		x = sorted[planets[a-1]] if planets[a-1] == arr[a-1]
# # 		planets[a-1] = x if x != -1
# # 		planets[b-1] = x if x != -1
# # 	end
# # 	puts planets.map { |e| arr.index(e) + 1 }.join(" ")
...
# 	(n-1).times do 
# 		a,b = gets.split(" ").map(&:to_i)
# 		planets[a] ||= MaxHeap.new(arr)
# 		planets[b] ||= MaxHeap.new(arr)
# 		planets[a].delete(arr[b-1])
# 		planets[a].delete(arr[a-1])
# 		planets[b].delete(arr[b-1])
# 		planets[b].delete(arr[a-1])
# 	end

# 	m = arr.index(arr.max) + 1
# 	ans = [m] * n

# 	puts ans.each_with_index { |e,i| planets[i+1] ? m : e }.join(" ")
...
t.times do 
	n = gets.to_i
	arr = gets.split(" ").map(&:to_i)
	indices =  Hash[[*arr.each_with_index]]
	...
	(n-1).times do 
		...
		planets[a] ||= []
		planets[b] ||= []
		planets[a] << arr[a-1] << arr[b-1]
		planets[b] << arr[a-1] << arr[b-1]
	end
	...
	planets.each do |k,v|
		sorted.each do |ind|
			unless v.include?(ind)
				ans[k-1] = arr.index(ind) + 1
				break
			end
		end
	end
	puts ans.each { |e| e }.join(" ")
